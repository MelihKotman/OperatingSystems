...
// mutex_sem: !!! Mutex yerine geçen Binary Semafor !!!
sem_t e, f,mutex_sem;
...
void *uretici(void *arg) {
    ...
    myID = (int)arg;
        ...
        // 2. Adım: Kritik Bölgeye giriş için Mutex'i(veya Binary Semafor) al.
        // Aynı anda başka bir üretici veya tüketici x'i değiştiremesin.
        sem_wait(&mutex_sem); // Başlangıçta 1'dir. wait yapınca 0 olur ve kilitlenir.
        ...
        // 3. Adım: Mutex'i(veya Binary Semafor) bırak.
        //pthread_mutex_unlock(&kilit);
        sem_post(&mutex_sem); // post yapınca 0'dan 1'e çıkar ve kilit açılır.
...
int main(int argc, char **argv) {
    ...
    // !!! MUTEX YERİNE SEMAFOR BAŞLATMA !!!
    // Başlangıç değeri 1 olmalı ki ilk gelen girebilsin.
    // 1 = Kilit Açık, 0 = Kilitli
 ...
 * - Aslında Binary Semafor olan 'mutex_sem'de aynı işi yapar eğer sadece semafor derse
 * böyle çözmek gerekir fakat mutexli isterse çözebilirsin. Her ikisi de aynı yola çıkar.
 *
 * 🎯 Sonuç:
